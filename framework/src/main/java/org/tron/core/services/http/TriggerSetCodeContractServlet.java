...
import org.tron.protos.contract.SmartContractOuterClass.SetCodeAuthorization.Authorization;
import org.tron.protos.contract.SmartContractOuterClass.SetCodeContract;

@Component
@Slf4j(topic = "API")
public class TriggerSetCodeContractServlet extends RateLimiterServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
      throws IOException {
    SetCodeContract.Builder build = SetCodeContract.newBuilder();
    ...
    try {
      ...
      JSONArray auths = Util.getJsonArray(jsonObject, "auths", true);
      for (int i = 0; i < auths.size(); i++) {
        SetCodeAuthorization.Builder setCodeAuthBuilder = SetCodeAuthorization.newBuilder();
        JSONObject setCodeAuthorization = auths.getJSONObject(i);
        JSONObject auth = setCodeAuthorization.getJSONObject("auth");

        Authorization.Builder authBuilder = Authorization.newBuilder();
        authBuilder.setChainId(Util.getJsonLongValue(auth, "chain_id"));
        authBuilder.setAddress(ByteString.fromHex(setCodeAuthorization.getString("address")));
      ...
      Transaction trx = wallet
          .createTransactionCapsule(build.build(), ContractType.SetCodeContract).getInstance();
