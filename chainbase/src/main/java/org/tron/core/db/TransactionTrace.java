  ...
  public void pay() throws BalanceInsufficientException {
    ...
    switch (trxType) {
      ...
      case TRX_CONTRACT_SET_CODE_TYPE:
        SetCodeContract setCodeContract = ContractCapsule
            .getSetCodeContractFromTransaction(trx.getInstance());
        contractCapsule =
            contractStore.get(setCodeContract.getContractAddress().toByteArray());

        callerAccount = setCodeContract.getOwnerAddress().toByteArray();
        disableJavaLangMath = dynamicPropertiesStore.disableJavaLangMath();
